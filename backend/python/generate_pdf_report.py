...
# ============================================================
# UNICODE FONT REGISTRATION (Turkish character support)
# ============================================================
_FONT_REGISTERED = False

def register_unicode_fonts():
    """Register DejaVu Sans fonts for Turkish/Unicode character support."""
    ...
    if regular and bold:
        ...
        print("WARNING: DejaVu Sans fonts not found. Turkish characters may not render correctly.")


# Font name constants — after registration, these hold the correct name
FONT_REGULAR = 'Helvetica'
FONT_BOLD = 'Helvetica-Bold'

def _apply_font_names():
    """Update font constants after registration."""
...
# ============================================================
# COLOR PALETTE — GeoSerra Brand (Emerald/Cyan)
# ============================================================
PRIMARY = HexColor("#06090f")       # GeoSerra dark background
SECONDARY = HexColor("#0f1420")     # Card background
ACCENT = HexColor("#10b981")        # Emerald — GeoSerra primary
HIGHLIGHT = HexColor("#0ea5e9")     # Cyan — GeoSerra secondary
SUCCESS = HexColor("#10b981")       # Emerald green
WARNING = HexColor("#f59e0b")       # Amber
DANGER = HexColor("#ef4444")        # Red
INFO = HexColor("#0ea5e9")          # Cyan
LIGHT_BG = HexColor("#f0fdf4")      # Emerald tint background
MEDIUM_BG = HexColor("#d1fae5")     # Emerald medium background
TEXT_PRIMARY = HexColor("#1e293b")  # Dark text
TEXT_SECONDARY = HexColor("#64748b") # Slate grey
WHITE = white
BLACK = black

# Logo path — priority: storage/assets/ > script dir > fallback
import os as _os
_SCRIPT_DIR = _os.path.dirname(_os.path.abspath(__file__))
_STORAGE_LOGO = _os.path.join(_SCRIPT_DIR, "..", "storage", "assets", "logo.png")
_SCRIPT_LOGO  = _os.path.join(_SCRIPT_DIR, "logo-small.png")
_SCRIPT_LOGO2 = _os.path.join(_SCRIPT_DIR, "logo.png")

if _os.path.exists(_STORAGE_LOGO):
    LOGO_PATH = _os.path.abspath(_STORAGE_LOGO)
elif _os.path.exists(_SCRIPT_LOGO):
    LOGO_PATH = _SCRIPT_LOGO
elif _os.path.exists(_SCRIPT_LOGO2):
    LOGO_PATH = _SCRIPT_LOGO2
else:
    LOGO_PATH = ""
...
def header_footer(canvas, doc):
    """Add GeoSerra branded header and footer to each page."""
    canvas.saveState()

    # Header — emerald accent bar
    canvas.setFillColor(ACCENT)
    canvas.rect(50, letter[1] - 44, letter[0] - 100, 3, fill=1, stroke=0)

    # Header logo (if available)
    if _os.path.exists(LOGO_PATH):
        try:
            canvas.drawImage(LOGO_PATH, 50, letter[1] - 38, width=90, height=22,
    ...
    canvas.drawRightString(letter[0] - 50, letter[1] - 32, "GeoSerra — GEO SEO Raporu")
    ...
    canvas.setFont(FONT_REGULAR, 8)
    canvas.setFillColor(TEXT_SECONDARY)
    canvas.drawString(50, 28, f"geoserra.com  |  {datetime.now().strftime('%d.%m.%Y')}")
    canvas.drawRightString(letter[0] - 50, 28, f"Sayfa {doc.page}")
    canvas.drawCentredString(letter[0] / 2, 28, "Gizli — Yalnızca Müşteriye Özel")
...
def generate_report(data, output_path="GEO-REPORT.pdf"):
    ...
    doc = SimpleDocTemplate(
        ...
        leftMargin=50,
        rightMargin=50,
        title=f"GeoSerra GEO SEO Raporu — {data.get('url', '')}",
        author="GeoSerra — geoserra.com",
        subject="GEO SEO Analiz Raporu",
        creator="GeoSerra Platform",
        keywords="GEO SEO AI görünürlük analizi ChatGPT Gemini Perplexity",
    )
    ...
    if _os.path.exists(LOGO_PATH):
        try:
            ...
            logo_img = RLImage(LOGO_PATH, width=160, height=42)
    ...
    # Title
    elements.append(Paragraph("GEO SEO Analiz Raporu", styles['ReportTitle']))
    elements.append(Spacer(1, 8))

    # Subtitle
    elements.append(Paragraph(
        f"<b>{brand_name}</b> için Yapay Zeka & SEO Görünürlük Analizi",
    ...
    elements.append(Paragraph(
        "Bu rapor GeoSerra platformu (geoserra.com) tarafindan otomatik olarak olusturulmustur. "
        "Skorlar ve oneriler otomatik analiz ile sektor kiyaslamalari esas alinarak hesaplanmaktadir. "
        "Sonuclarin platform bazli testlerle dogrulanmasi onerilir. "
        "(c) GeoSerra ",
        styles['SmallText']
    ))
